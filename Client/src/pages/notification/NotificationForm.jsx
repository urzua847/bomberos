...
import { UseNotification } from "../../context/RecordContext";
...
function NotificationForm() {
  ...
  const {createNotification, getOneNotification, updateNotification} = UseNotification();
  ...
  useEffect(() => {
    async function loadNotification(){
      if(params.id){ 
        const Notification = await getOneNotification(params.id)
        console.log(Notification)
        setValue('name', Notification.name);
        setValue('lastname', Notification.lastname);
        setValue('phone', Notification.phone);
      }
    }
    loadNotification();
  },[params.id, getOneNotification, setValue]);

  const onSubmit = handleSubmit((data) =>{
    if(params.id){
      updateNotification(params.id, data);
    }else{
